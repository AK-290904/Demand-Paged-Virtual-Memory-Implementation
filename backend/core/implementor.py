...
class VirtualMemoryImplementor:
    ...
    def run_simulation(self) -> SimulationResult:
        ...
        try:
            ...
            while not self.scheduler.is_simulation_complete():
                ...
                # Get next page reference and access type (READ/WRITE)
                page_number = process.reference_string[process.current_index]
                access_type = "READ"
                if process.access_types and process.current_index < len(process.access_types):
                    access_type = process.access_types[process.current_index]
                
                # Handle page request
                frame_number, event_type, message = self.mmu.handle_page_request(
                    process_id, page_number, access_type
                )
                ...
                elif event_type == "protection_fault":
                    # Handle Copy-on-Write protection fault
                    print(f"[PROTECTION] {message}")
                    frame_number = self.mmu.handle_protection_fault(process_id, page_number)
                    
                    # Create step for CoW handling
                    cow_step = self._create_step(
                        process_id, page_number, frame_number,
                        "cow_handled",
                        f"Copy-on-Write Handled: P{process_id} -> Page {page_number} -> Frame {frame_number}"
                    )
                    self.steps.append(cow_step)
                    
                    # After CoW, treat as a successful access and advance
    ...
    def _create_step(self, process_id: int, page_number: int, 
        ...
        # Update sliding window of recent faults for thrashing detection
        # Treat both page_fault and protection_fault (CoW) as "fault" events.
        is_fault_event = event_type in ("page_fault", "protection_fault")
        self._fault_history.append(is_fault_event)
        if len(self._fault_history) > 20:
            self._fault_history.pop(0)

        thrashing_detected = False
        if len(self._fault_history) >= 20:
            fault_count = sum(1 for x in self._fault_history if x)
            fault_rate = (fault_count / len(self._fault_history)) * 100.0
            thrashing_detected = fault_rate > 70.0

        # Compute shared frames mapping: frame_id -> [list of PIDs]
        # Only compute if shared memory is enabled
        shared_frames: Dict[int, List[int]] = {}
        if self.config and self.config.enable_shared_memory:
            frame_to_pids: Dict[int, List[int]] = {}
            for pid, process in enumerate(self.processes):
                for page_idx in range(process.m):
                    pt_entry = self.mmu.page_table[pid][page_idx]
                    if pt_entry.valid:
                        frame = pt_entry.frame
                        if frame not in frame_to_pids:
                            frame_to_pids[frame] = []
                        if pid not in frame_to_pids[frame]:
                            frame_to_pids[frame].append(pid)

            shared_frames = {
                frame: pids for frame, pids in frame_to_pids.items() if len(pids) > 1
            }
