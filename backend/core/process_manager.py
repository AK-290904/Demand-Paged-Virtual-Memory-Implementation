...
class ProcessManager:
    ...
    def create_processes(num_processes: int, max_pages: int, num_frames: int) -> List[Process]:
        ...
        for process in processes:
            ...
            # Generate corresponding access types so that some references are writes.
            # This allows the MMU to exercise Copy-on-Write behavior.
            process.access_types = [
                "WRITE" if random.random() < 0.3 else "READ"
                for _ in process.reference_string
            ]
    ...
    def create_processes_with_references(num_processes: int, max_pages: int, 
        ...
        # For provided reference strings, generate a simple READ/WRITE pattern
        # so that Copy-on-Write behavior can still be demonstrated.
        for process in processes:
            process.access_types = [
                "WRITE" if random.random() < 0.3 else "READ"
                for _ in process.reference_string
